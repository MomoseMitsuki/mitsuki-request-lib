...
export function createCacheRequestor(
	...
	const wrap = (send: Requestor["_request"]) => {
		return async (url: string, method: RequestMethod, option: RequestOptions = {}) => {
			...
			option.pathname = url;
			const cached = (await req.emitAsync("beforeRequest", option)) as ResponseLike | void;
			// 捕获缓存
			if (cached) {
				return cached;
			}
	...
	newRequestor.on("beforeRequest", async (config: RequestOptions) => {
		const key = options.key(config);
		const dataKey = `${key}#data`;
		const hasKey = await store.has(dataKey);
		// todo: isVaild 验证
		const isVaild = await options.isVaild(key, config);
		if (hasKey && isVaild) {
...
export function createSerialRequestor() {
	return createParalleRequestor(1);
}
...
function normalizeOptions(cacheOptions: CacheOptions): Required<CacheOptions> {
	if (!cacheOptions.isVaild) {
		const isVaild = async (key: string, config: RequestOptions) => {
			const stores = useCacheStore(cacheOptions.persist);
			const dataKey = `${key}#data`;
			const timeKey = `${key}#time`;
